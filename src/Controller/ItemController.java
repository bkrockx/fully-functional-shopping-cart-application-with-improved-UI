...
public class ItemController {
	...
	public String getAddToUser(@RequestParam("bid")String userName,@RequestParam("cid")Integer itemId,Model model){
		...
		List<useritem> userItems = itemService.getUserItems(user.getUserId());
		List<Item> items = new ArrayList<Item>();
		 
		int sum = 0;
		for (useritem it2 : userItems) {
			items.add(it2.getItem());
			sum = sum + it2.getItem().getItemPrice();
		}
	 ...
	 @RequestMapping(value="/checkOut",method = RequestMethod.POST)
		 ...
		 */
		 ...
		 for(useritem ui:userItems){
			...
			// items1.add(ui.getItem());
			 sum = sum + ui.getItem().getItemPrice();
		 }
